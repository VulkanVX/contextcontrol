    ...
    public string PromptTokenomicsLabel
    {
        get
        {
            ...
            if (IsCodexPromptMode)
            {
                ...
                {
                    return $"Current prompt tok: {codexPromptTokens:N0} | {codexPromptTokens:N0} prompt tok | 0 attachment tok";
                }
    ...
    public string PromptContextPressureLabel
    {
        get
        {
            ...
            if (IsCodexPromptMode)
            {
                if (!IsAutopilotEnabled)
                {
                    var rawPromptTokens = ContextCapsuleBuilder.EstimateTokens(PromptText);
                    return $"Codex CLI raw: {rawPromptTokens:N0} prompt tok est; no ContextControl harness";
                }
