...
public sealed partial class ChatTranscriptRenderControl
{
    private bool TryGetTextPosition(Point point, out TextPosition position)
    {
        ...
        foreach (var selectable in layout.SelectableTextBlocks)
        {
            ...
            var scrollOffset = GetSelectableScrollOffset(layout, selectable);
    ...
    private bool TryScrollExtensionPanel(Point point, double deltaY)
    {
        ...
        if (layout.Thinking is { TextBlock: { } thinkingBlock } thinking
            ...
            && ScrollExtensionPanel(thinking.Hit.Parameter, thinkingBlock, _thinkingScrollOffsets, deltaY))
        {
            InvalidateVisual();
            return true;
        }
        ...
        if (layout.Diagnostic is { TextBlock: { } diagnosticBlock } diagnostic
            ...
            && ScrollExtensionPanel(diagnostic.Hit.Parameter, diagnosticBlock, _diagnosticScrollOffsets, deltaY))
        {
            InvalidateVisual();
            return true;
        }
        ...
        foreach (var snippet in layout.Snippets)
        {
            ...
            if (snippet.CodeTextBlock is { TextBlock: { } snippetBlock }
                && OffsetY(snippet.CodeRect, rowTop).Contains(point)
                && ScrollExtensionPanel(snippet.Snippet, snippetBlock, _snippetScrollOffsets, deltaY))
            {
                InvalidateVisual();
                return true;
            }
    ...
    private double GetSelectableScrollOffset(MessageLayout layout, SelectableTextBlockLayout selectable)
    {
        if (layout.Thinking is { SelectableTextBlock: { } thinkingBlock, TextBlock: { } thinkingText }
            && ReferenceEquals(thinkingBlock, selectable))
        {
            return GetExtensionScrollOffset(layout.Thinking.Hit.Parameter, thinkingText, _thinkingScrollOffsets);
        }

        if (layout.Diagnostic is { SelectableTextBlock: { } diagnosticBlock, TextBlock: { } diagnosticText }
            && ReferenceEquals(diagnosticBlock, selectable))
        {
            return GetExtensionScrollOffset(layout.Diagnostic.Hit.Parameter, diagnosticText, _diagnosticScrollOffsets);
        }

        foreach (var snippet in layout.Snippets)
        {
            if (ReferenceEquals(snippet.CodeTextBlock, selectable))
            {
                return GetExtensionScrollOffset(snippet.Snippet, selectable.TextBlock, _snippetScrollOffsets);
            }
        }
    ...
    private bool ScrollExtensionPanel(
        ...
        Dictionary<object, double> offsets,
        double deltaY)
    {
        if (key is null)
        {
        ...
        offsets.TryGetValue(key, out var current);
    ...
    private bool TryResolveExtensionScrollbar(Point point, out ExtensionScrollbarHit scrollbar)
    {
        ...
        if (layout.Thinking is { TextBlock: { } thinkingBlock } thinking
            && TryResolveExtensionScrollbar(point, rowTop, thinking.Hit.Parameter, thinking.BodyRect, thinkingBlock, _thinkingScrollOffsets, out scrollbar))
        {
            return true;
        }

        if (layout.Diagnostic is { TextBlock: { } diagnosticBlock } diagnostic
            && TryResolveExtensionScrollbar(point, rowTop, diagnostic.Hit.Parameter, diagnostic.BodyRect, diagnosticBlock, _diagnosticScrollOffsets, out scrollbar))
        {
            return true;
        }
        ...
        foreach (var snippet in layout.Snippets)
        {
            ...
            if (snippet.CodeTextBlock is { TextBlock: { } snippetBlock }
                && TryResolveExtensionScrollbar(point, rowTop, snippet.Snippet, snippet.CodeRect, snippetBlock, _snippetScrollOffsets, out scrollbar))
            {
                return true;
            }
        }
        ...
        out ExtensionScrollbarHit scrollbar)
    {
        ...
        var scrollOffset = GetExtensionScrollOffset(key, block, offsets);
        ...
        scrollbar = new ExtensionScrollbarHit(key, offsets, track.Y, track.Height, thumbHeight, max, track.Y + thumbOffset);
        return true;
    }
    ...
    private bool UpdateExtensionScrollbarDrag(double pointerY)
    {
        ...
        drag.Offsets[drag.Key] = ratio * drag.MaxOffset;
        InvalidateVisual();
        return true;
    }
