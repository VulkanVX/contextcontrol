...
public sealed partial class ChatTranscriptRenderControl
{
        ...
        double viewportBottom)
    {
        ...
        {
            DrawThinking(context, layout.Thinking, index, rowTop, viewportTop, viewportBottom);
        }

        if (layout.Diagnostic is not null)
        {
            DrawDiagnostic(context, layout.Diagnostic, index, rowTop, viewportTop, viewportBottom);
        }
    }
    ...
    private void DrawSnippet(DrawingContext context, SnippetLayout snippet, int rowIndex, double rowTop, double viewportTop, double viewportBottom)
    {
        ...
        {
            var block = snippet.CodeTextBlock.TextBlock;
            var scrollOffset = GetExtensionScrollOffset(snippet.Snippet, block, _snippetScrollOffsets);
            DrawTextSelection(context, block, rowIndex, snippet.CodeTextBlock.BlockIndex, rowTop, viewportTop, viewportBottom, scrollOffset);
            DrawScrollableTextBlock(context, block, scrollOffset, rowTop, viewportTop, viewportBottom);
            DrawExtensionScrollbar(context, code, block, scrollOffset);
        }
    }
    ...
    private void DrawDiagnostic(DrawingContext context, DiagnosticLayout diagnostic, int rowIndex, double rowTop, double viewportTop, double viewportBottom)
    {
        ...
        context.DrawRectangle(
            ...
            3,
            3);
        var scrollOffset = GetExtensionScrollOffset(diagnostic.Hit.Parameter, diagnostic.TextBlock, _diagnosticScrollOffsets);
        if (diagnostic.SelectableTextBlock is not null)
        {
            DrawTextSelection(context, diagnostic.TextBlock, rowIndex, diagnostic.SelectableTextBlock.BlockIndex, rowTop, viewportTop, viewportBottom, scrollOffset);
        }
        ...
        using (context.PushClip(rect))
        {
            for (var lineIndex = 0; lineIndex < block.Lines.Count; lineIndex++)
            {
                ...
                var highlight = new Rect(
                    rect.X + prefixWidth,
                    rect.Y - scrollOffset + lineIndex * block.LineHeight,
