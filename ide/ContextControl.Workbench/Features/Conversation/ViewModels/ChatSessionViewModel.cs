...
public sealed class ChatSessionViewModel : ObservableObject
{
    ...
    private string _draftPromptModeKey;
    private bool? _draftIsAutopilotEnabled;
    private string _workflowTaskText;
    private DateTime _updatedUtc;
    private bool _isActive;
    private bool _isGenerating;
    private bool _hasNewResponse;
    ...
    public ChatSessionViewModel(ChatHistorySessionData data)
    {
        ...
        _draftPromptModeKey = NormalizeDraftPromptModeKey(data.DraftPromptModeKey);
        _draftIsAutopilotEnabled = data.DraftIsAutopilotEnabled;
    ...
    public bool IsActive
    {
        ...
        set
        {
            ...
            {
                OnPropertyChanged(nameof(ShowNewResponseHighlight));
                OnPropertyChanged(nameof(ShowGeneratingLoader));
    ...
    public bool HasNewResponse
    {
        get => _hasNewResponse;
        private set
        {
            if (SetProperty(ref _hasNewResponse, value))
            {
                OnPropertyChanged(nameof(ShowNewResponseHighlight));
                OnPropertyChanged(nameof(ShowGeneratingLoader));
                OnPropertyChanged(nameof(ShowUnreadDot));
    ...
    public bool ShowNewResponseHighlight => HasNewResponse && !IsActive;

    public bool ShowGeneratingLoader => IsGenerating && !ShowUnreadDot;

    public bool ShowUnreadDot => HasNewResponse && !IsActive;

    public int MessageCount => _messages.Count;

    public string UpdatedLabel => UpdatedUtc.ToLocalTime().ToString("MMM d HH:mm");

    public string RouteLabel => ResolveRouteLabel();

    public string Summary => $"{MessageCount:N0} msg - {RouteLabel} - {UpdatedLabel}";
    ...
    public string DraftPromptModeKey => _draftPromptModeKey;
    public bool? DraftIsAutopilotEnabled => _draftIsAutopilotEnabled;
    ...
    public static ChatSessionViewModel CreateNew()
    {
        ...
        return new ChatSessionViewModel(new ChatHistorySessionData
        {
            ...
            CreatedUtc = now,
            UpdatedUtc = now,
            DraftPromptModeKey = "context",
            DraftIsAutopilotEnabled = false
        });
    }
    ...
    public string FindLastDiagnosticPrompt()
    {
        for (var index = _messages.Count - 1; index >= 0; index--)
        {
            var prompt = _messages[index].DiagnosticPrompt;
    ...
    public void SetPromptDraft(string? text, string? promptModeKey, bool? isAutopilotEnabled)
    {
        _draftPromptText = text ?? "";
        _draftPromptModeKey = NormalizeDraftPromptModeKey(promptModeKey);
        _draftIsAutopilotEnabled = isAutopilotEnabled;
    ...
    public void MarkNewResponse()
    {
        HasNewResponse = true;
    }

    public void ClearNewResponse()
    {
    ...
    public ChatHistorySessionData ToData()
    {
        return new ChatHistorySessionData
        {
            ...
            DraftPromptModeKey = _draftPromptModeKey,
            DraftIsAutopilotEnabled = _draftIsAutopilotEnabled,
    ...
    private static string NormalizeDraftPromptModeKey(string? value)
    {
    ...
    private string ResolveRouteLabel()
    {
        ...
        return string.Equals(_draftPromptModeKey, "codex", StringComparison.OrdinalIgnoreCase)
            ? "Codex CLI"
            : "Local";
    }

    private static bool IsCodexMessage(ChatHistoryMessageData message)
    {
        return (message.Phase ?? "").StartsWith("Codex", StringComparison.OrdinalIgnoreCase)
            || (message.ModelId ?? "").Contains("codex", StringComparison.OrdinalIgnoreCase);
    }
