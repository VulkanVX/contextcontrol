...
public sealed partial class LocalLlmChatMessageViewModel : ObservableObject
{
    ...
    private string _liveThinkingTargetText = "";
    private int _liveVisibleTextLength;
    private int _liveThinkingVisibleTextLength;
    private int _liveWaitingFrameIndex;
    private bool _isLiveWaitingActive;
    private bool _isLiveThinkingPlaceholderActive;
    private LocalLlmUsageStats? _stats;
    private bool _isThinkingExpanded;
    private bool _isDiagnosticExpanded;
    private static readonly string[] LiveWaitingFrames =
    [
        "Thinking.",
        "Thinking..",
        "Thinking..."
    ...
    public LocalLlmChatMessageViewModel(
        ...
        bool canShowLiveThinkingPlaceholder = false)
    {
        ...
        _liveWaitingTimer.Tick += (_, _) => TickLiveWaiting();
    ...
    public void UpdateLiveStatus(string status)
    {
        ...
        StopLiveWaiting();
        QueueLiveVisibleText(clean);
    }

    public void AppendLiveThinking(string thinkingDelta)
    {
        var clean = NormalizeLiveThinkingText(thinkingDelta);
        if (string.IsNullOrEmpty(clean))
        {
            return;
        }

        ClearLiveThinkingPlaceholder();
        if (string.IsNullOrEmpty(_liveThinkingTargetText))
        {
            _liveThinkingTargetText = _thinkingText;
    ...
    {
        _canShowLiveThinkingPlaceholder = false;
        ClearLiveThinkingPlaceholder();
    }

    public void UpdateLiveThinking(string thinkingText)
    {
        var clean = NormalizeLiveThinkingText(thinkingText);
        if (string.IsNullOrEmpty(clean))
        {
            return;
        }
        ...
        ClearLiveThinkingPlaceholder();
        if (string.Equals(_liveThinkingTargetText, clean, StringComparison.Ordinal)
            && _liveThinkingVisibleTextLength >= clean.Length)
        {
            return;
        }

        var visiblePrefix = _liveThinkingVisibleTextLength > 0 && _liveThinkingVisibleTextLength <= _liveThinkingTargetText.Length
            ? _liveThinkingTargetText[.._liveThinkingVisibleTextLength]
            : _thinkingText;
    ...
    {
        if (!_canShowLiveThinkingPlaceholder)
        {
            ClearLiveVisibleText();
            return;
        }

        if (!_isLiveWaitingActive)
        {
            _isLiveWaitingActive = true;
            _liveWaitingFrameIndex = 0;
            ClearLiveVisibleText();
            ApplyLiveThinkingPlaceholderFrame(LiveWaitingFrames[_liveWaitingFrameIndex]);
        }

        if (!_liveWaitingTimer.IsEnabled)
        {
            _liveWaitingTimer.Start();
        }
    }

    private void TickLiveWaiting()
    {
        if (!_isLiveWaitingActive && !_isLiveThinkingPlaceholderActive)
        {
            _liveWaitingTimer.Stop();
            return;
        }

        _liveWaitingFrameIndex = (_liveWaitingFrameIndex + 1) % LiveWaitingFrames.Length;
        var frame = LiveWaitingFrames[_liveWaitingFrameIndex];
        if (_isLiveThinkingPlaceholderActive)
        {
            ApplyLiveThinkingPlaceholderFrame(frame);
        }
    }

    private void StopLiveWaiting()
    {
        if (!_isLiveWaitingActive && !_liveWaitingTimer.IsEnabled)
        {
            return;
        }
    ...
    private void StopLivePresentation()
    {
        ...
        _liveWaitingTimer.Stop();
        _liveTargetText = "";
        _liveThinkingTargetText = "";
        _liveVisibleTextLength = 0;
        _liveThinkingVisibleTextLength = 0;
    }

    private static bool IsLiveWaitingText(string text)
    {
        return LiveWaitingFrames.Any(frame => frame.Equals((text ?? "").Trim(), StringComparison.OrdinalIgnoreCase));
    }

    private void ClearLiveVisibleText()
    {
        _rawText = "";
        _visibleText = "";
        _liveTargetText = "";
    ...
    private void ApplyLiveThinkingPlaceholderFrame(string text)
    {
        if (!_isLiveThinkingPlaceholderActive)
        {
            _isLiveThinkingPlaceholderActive = true;
            _liveThinkingTargetText = "";
            _liveThinkingVisibleTextLength = 0;
        }
    ...
    private void ClearLiveThinkingPlaceholder()
    {
        if (!_isLiveThinkingPlaceholderActive)
        {
            return;
    ...
    private static string NormalizeLiveThinkingText(string text)
    {
        var clean = (text ?? "")
    ...
    private void ApplyLiveVisibleText(string text)
    {
        _rawText = text ?? "";
        var parsed = ParseMessage(_rawText, ShouldExtractRequestSnippets(Role, Phase, _rawText));
        _visibleText = parsed.VisibleText;
