...
public sealed class ChatSnippetViewModel(string kind, string language, string text, string suggestedFileName = "") : ObservableObject
{
    ...
    public bool HasCreatedFile => !string.IsNullOrWhiteSpace(CreatedFilePath);
    public string CreatedFilePath
    {
        get => _createdFilePath;
        private set
        {
            if (SetProperty(ref _createdFilePath, value ?? ""))
            {
                OnPropertyChanged(nameof(HasCreatedFile));
                OnPropertyChanged(nameof(TypeLabel));
                OnPropertyChanged(nameof(Title));
                OnPropertyChanged(nameof(MetaLabel));
                OnPropertyChanged(nameof(CompactMetaLabel));
            }
        }
    }
    ...
    public string TypeLabel => IsPatch
        ? "patch"
        : IsRequestList ? "request" : IsPatchPlan ? "plan" : CanCreateProjectFile ? "file" : Language.ToLowerInvariant();

    public string Title => IsPatch
        ? PatchLabel
        : IsRequestList ? ResolveRequestMetaLabel() : IsPatchPlan ? "Patch file plan" : CanCreateProjectFile ? $"File snippet: {SuggestedFileName}" : $"Code snippet: {Language}";
    ...
    public string CompactMetaLabel
    {
        get
        {
            ...
            if (IsCode && HasCreatedFile)
            {
                return $"created {Path.GetFileName(CreatedFilePath)}";
            }

            if (IsCode && CanCreateProjectFile)
            {
