...
public sealed partial class ContextControlViewModel
{
    ...
    private void RestoreProjectPromptState(ChatHistoryDocument document)
    {
        ...
        {
            _settings.SelectedImageModel = selectedImageModelId;
    ...
    private void SelectChatSession(ChatSessionViewModel? session, bool save)
    {
        ...
        if (SelectedChatSession is { } previous && !ReferenceEquals(previous, session))
        {
            ...
            previous.SetPromptDraft(PromptText, ResolvePromptModeKeyForHistory(), IsAutopilotEnabled);
        }
        ...
        try
        {
            ...
            SkillbookCodexCapsuleText = session.FindLastDiagnosticPrompt();
    ...
    private void LoadPromptDraftForSession(ChatSessionViewModel session)
    {
        ...
        try
        {
            ...
            PromptText = draft;
            PromptModeKey = ResolvePromptModeKeyForSession(session);
            IsAutopilotEnabled = session.DraftIsAutopilotEnabled ?? false;
            session.SetPromptDraft(PromptText, ResolvePromptModeKeyForHistory(), IsAutopilotEnabled);
        }
        finally
        {
    ...
    private void SavePromptDraftToSelectedChat()
    {
        ...
        SelectedChatSession?.SetPromptDraft(PromptText, ResolvePromptModeKeyForHistory(), IsAutopilotEnabled);
    }

    private void SchedulePromptDraftSave()
    {
        if (_isSyncingChatDraft
            || _isSwitchingProjectState
            || _isSwitchingChatSession
            || _isSwitchingConversationKind)
        {
            return;
        }

        _promptDraftSaveTimer.Stop();
        _promptDraftSaveTimer.Start();
    }
    ...
    public void RenameChatSession(ChatSessionViewModel? session, string? title)
    {
        if (session is null || !ChatSessions.Contains(session))
        {
            return;
    ...
    private void MarkGeneratedResponseIfReady(ChatSessionViewModel session, LocalLlmChatMessageViewModel message)
    {
        if (!message.IsUser && !IsLiveAssistantPlaceholder(message) && !session.IsActive)
        {
            session.MarkNewResponse();
        }
    }

    private static bool IsLiveAssistantPlaceholder(LocalLlmChatMessageViewModel message)
    {
        return (message.CapsuleSummary ?? "").TrimStart().StartsWith("live ", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private void SaveChatHistory()
    {
        ...
        try
        {
            ...
            _chatHistoryService.Save(new ChatHistoryDocument
            {
                ...
                PromptText = "",
                PromptModeKey = ResolvePromptModeKeyForHistory(),
    ...
    private string ResolvePromptModeKeyForSession(ChatSessionViewModel session)
    {
        ...
        var clean = NormalizePromptModeKey(session.DraftPromptModeKey);
        return string.Equals(clean, "imagegen", StringComparison.OrdinalIgnoreCase)
            ? "context"
            : clean;
    }

    private string ResolvePromptModeKeyForHistory()
    {
        var clean = NormalizePromptModeKey(PromptModeKey);
        if (!string.Equals(clean, "terminal", StringComparison.OrdinalIgnoreCase))
        {
            return clean;
        }

        if (SelectedChatSession is { } session && !string.IsNullOrWhiteSpace(session.DraftPromptModeKey))
        {
            var sessionMode = NormalizePromptModeKey(session.DraftPromptModeKey);
            if (!string.Equals(sessionMode, "terminal", StringComparison.OrdinalIgnoreCase))
            {
                return sessionMode;
            }
        }

        var settingsMode = NormalizePromptModeKey(_settings.PromptModeKey);
        if (!string.Equals(settingsMode, "terminal", StringComparison.OrdinalIgnoreCase)
            && string.Equals(ResolveConversationKindForPromptMode(settingsMode), _activeConversationKind, StringComparison.OrdinalIgnoreCase))
        {
            return settingsMode;
        }

        return IsImageGenConversationKind(_activeConversationKind) ? "imagegen" : "context";
    }
