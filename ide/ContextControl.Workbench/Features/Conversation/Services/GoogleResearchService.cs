...
public static partial class GoogleResearchService
{
    ...
    public static string PlannerText(LocalLlmChatResult result)
    {
        // A verbose small planner can exhaust its short output budget. Its bounded JSON parser
        // can still use a complete object, or the normal query/number fallback. This is never a final answer.
        if (result.Succeeded || result.OutputLimited) return result.Message ?? "";
        throw new InvalidOperationException("The local research planner failed: " + result.Status);
    }
    ...
    private static string? FallbackQuery(string question) => PhotoSubject(question) is { } subject ? subject + " photo"
        : ExplicitSearch().IsMatch(question) || (SearchIntent().IsMatch(question) && GoogleKnowledgeRecovery.IsPublicLookupQuestion(question))
            ? GoogleSearchContext.NormalizeQuery(question) : null;
