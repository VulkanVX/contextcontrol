    ...
    public static string Build(string html)
    {
        ...
        var instrumentation = """
            ...
            (() => {
              ...
              addEventListener('error',e=>report(e.message || 'A game resource could not load.'),true);
            ...
            """;
        var child = "<!doctype html><meta charset='utf-8'><meta http-equiv='Content-Security-Policy' content=\"" + policy + "\">" + instrumentation + html;
        return """
            ...
            addEventListener('message',e=>{
              if(e.source!==document.getElementById('game')?.contentWindow || !e.data || !['game-error','game-ready','game-stats'].includes(e.data.kind))return;
