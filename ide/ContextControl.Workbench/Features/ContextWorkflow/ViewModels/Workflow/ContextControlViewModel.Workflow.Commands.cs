...
public sealed partial class ContextControlViewModel
{
    ...
    private async Task RunDirTreeExportAsync()
    {
        ...
        {
            var outputPath = _processService.DirectoryTreeExportPath;
            ...
            var treeText = BuildCurrentProjectDirectoryTreeText(_projectTreeProvider?.Invoke() ?? []);
            await File.WriteAllTextAsync(outputPath, treeText, new UTF8Encoding(false));
            ...
            PatchSummary = "No patch loaded.";
            UpdatePatchPlanActions(null);
            AddAttachment(Path.GetFileName(outputPath), outputPath, "dir");
            LastExportPath = outputPath;
            PhaseTitle = "TREE ready";
            PhaseDetail = "Directory tree export attached with file names.";
            Log("ok", $"TREE export attached: {Path.GetFileName(outputPath)}");
            AppendTerminalOutput($"TREE export ready: {Path.GetFileName(outputPath)} attached from current non-skipped project tree.");
        });
    }

    private static string BuildCurrentProjectDirectoryTreeText(IEnumerable<ProjectNodeViewModel> roots)
    {
        var includedRoots = (roots ?? [])
            .Where(ShouldIncludeDirectoryTreeNode)
            .ToArray();
        ...
        for (var index = 0; index < includedRoots.Length; index++)
        {
            AppendProjectDirectoryTreeNode(builder, includedRoots[index], "", index == includedRoots.Length - 1, isRoot: true);
        }

        return builder.ToString().TrimEnd() + Environment.NewLine;
    }

    private static void AppendProjectDirectoryTreeNode(
        ...
        bool isRoot)
    {
        ...
        if (node.IsFolder)
        {
            ...
            {
                AppendProjectDirectoryTreeNode(builder, children[index], childPrefix, index == children.Length - 1, isRoot: false);
            }
    ...
    private static bool ShouldIncludeDirectoryTreeNode(ProjectNodeViewModel node)
    {
        if (node.IsExternal)
        {
            return false;
        ...
        return node.IsFile || node.Children.Any(ShouldIncludeDirectoryTreeNode);
    }
