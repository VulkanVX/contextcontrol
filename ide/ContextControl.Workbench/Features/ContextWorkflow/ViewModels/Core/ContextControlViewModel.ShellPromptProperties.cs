...
public sealed partial class ContextControlViewModel
{
    ...
    public string PromptModeKey
    {
        ...
        private set
        {
            ...
            var isTerminalMode = string.Equals(clean, "terminal", StringComparison.OrdinalIgnoreCase);
            var conversationKindChanged = !isTerminalMode
                && !string.Equals(
                    _activeConversationKind,
                    ResolveConversationKindForPromptMode(clean),
                    StringComparison.OrdinalIgnoreCase);
            if (isTerminalMode && !string.Equals(previous, "terminal", StringComparison.OrdinalIgnoreCase))
            {
                SelectedChatSession?.SetPromptDraft(PromptText, previous, IsAutopilotEnabled);
            }
            ...
            if (SetProperty(ref _promptModeKey, clean))
            {
                ...
                OnPropertyChanged(nameof(IsLocalThinkingToggleVisible));
                OnPropertyChanged(nameof(LocalThinkingToggleLabel));
    ...
    public string PromptWatermark => IsCodexPromptAuthBlocked
        ...
        : IsCodexPromptMode
            ? IsAutopilotEnabled
                ? "Ask Codex through the ContextControl DIR -> CC -> GO flow..."
                : "Ask Codex raw. Sends only this prompt text to Codex CLI..."
    ...
    public string PromptSendButtonToolTip => IsDirSendMissingRequestWarning
        ...
        : IsImageGenPromptMode
            ? "Generate image"
            : IsCodexPromptMode
                ? IsAutopilotEnabled ? "Send ContextControl capsule to Codex" : "Send raw prompt to Codex"
                : "Send";
    ...
    public string PromptModelCapabilityHint
    {
        get
        {
            ...
            return SelectedLocalModel?.IsImageModel == true
                ? "Accepts image input"
                : SelectedLocalModel?.SupportsNativeThinkingRequest == true
                    ? IsLocalThinkingEnabled ? "Thinking enabled" : "Thinking disabled"
                : SelectedLocalModel?.SupportsThinking == true
                    ? "Thinking output"
                : "";
        }
    }
        ...
        get
        {
            ...
            if (IsCodexPromptMode)
            {
                return IsAutopilotEnabled
                    ? $"{tokenLabel} | read-only harness | no repo browsing"
                    : $"{tokenLabel} | raw Codex CLI | no ContextControl harness";
            }
    ...
    public string CodexStatus
    {
        ...
        private set
        {
            if (SetProperty(ref _codexStatus, string.IsNullOrWhiteSpace(value) ? "Codex CLI status pending" : value))
            {
                OnPropertyChanged(nameof(PromptModelCapabilityHint));
                OnPropertyChanged(nameof(HasPromptModelCapabilityHint));
            }
