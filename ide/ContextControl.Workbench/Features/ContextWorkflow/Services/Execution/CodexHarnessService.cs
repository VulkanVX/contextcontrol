...
public sealed record CodexRawRequest(
    string UserMessage,
    string ContextControlRoot,
    string WorkingDirectory,
    string Model = "",
...
public sealed record CodexRawExecutionPlan(
    string WorkingDirectory,
    string LastMessagePath,
    IReadOnlyList<string> Arguments);
...
public sealed class CodexHarnessService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var rawPlan = BuildRawExecutionPlan(root, request.WorkingDirectory, request.Model, request.ReasoningEffort);
            Directory.CreateDirectory(Path.GetDirectoryName(rawPlan.LastMessagePath) ?? root);
            workingDirectory = rawPlan.WorkingDirectory;
            lastMessagePath = rawPlan.LastMessagePath;
            arguments = rawPlan.Arguments;
        }
        else
        {
        ...
        if (request.IsRawPrompt)
        {
            terminal?.Report("Codex raw mode sends only the user prompt; no ContextControl harness, Skillbook instructions, or attachments.");
        ...
        try
        {
            ...
            var stdoutTask = Task.Run(async () =>
            {
                while (!process.StandardOutput.EndOfStream)
                {
                    ...
                    if (parsed.IsTerminalTurn
                        && (messageCandidates.Count > 0 || File.Exists(lastMessagePath)))
                    {
            ...
            var finalMessage = await ReadLastMessageAsync(lastMessagePath, cancellationToken);
    ...
    public Task<CodexHarnessResult> SendRawAsync(
        CodexRawRequest request,
        IProgress<LocalLlmGenerationProgress>? progress = null,
        IProgress<string>? terminal = null,
        CancellationToken cancellationToken = default)
    {
        return SendAsync(
            new CodexHarnessRequest(
                request.UserMessage,
    ...
    public static CodexRawExecutionPlan BuildRawExecutionPlan(
        string contextControlRoot,
        string workingDirectory = "",
        string model = "",
        string reasoningEffort = "")
    {
        var root = string.IsNullOrWhiteSpace(contextControlRoot)
            ? AppContext.BaseDirectory
            : contextControlRoot;
        var resolvedWorkingDirectory = ResolveRawWorkingDirectory(root, workingDirectory);
        var rawRoot = Path.Combine(root, ".tmp", "codex-raw");
        var lastMessagePath = Path.Combine(rawRoot, "last-codex-raw-message.md");
        ...
        arguments.Add("-");
        return new CodexRawExecutionPlan(resolvedWorkingDirectory, lastMessagePath, arguments);
    }

    private static string ResolveRawWorkingDirectory(string contextControlRoot, string workingDirectory)
    {
        var root = string.IsNullOrWhiteSpace(contextControlRoot)
            ? AppContext.BaseDirectory
            : Path.GetFullPath(contextControlRoot);
