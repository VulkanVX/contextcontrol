...
public sealed partial class ContextControlViewModel
{
    ...
    public string SelectedLocalModelLabel => SelectedActiveLocalModel?.DisplayNameWithThinking ?? "No installed local model";

    public bool IsLocalThinkingEnabled
    {
        get => _isLocalThinkingEnabled;
        set
        {
            if (SetProperty(ref _isLocalThinkingEnabled, value))
            {
                OnPropertyChanged(nameof(LocalThinkingToggleLabel));
                OnPropertyChanged(nameof(PromptModelCapabilityHint));
                OnPropertyChanged(nameof(HasPromptModelCapabilityHint));
    ...
    public bool IsLocalThinkingToggleVisible => IsLocalPromptMode && SelectedLocalModel?.SupportsNativeThinkingRequest == true;

    public string LocalThinkingToggleLabel => IsLocalThinkingEnabled ? "Thinking on" : "Thinking off";

    private void NotifyLocalThinkingCapabilityChanged()
    {
        OnPropertyChanged(nameof(SelectedLocalModelLabel));
        OnPropertyChanged(nameof(SelectedActiveLocalModel));
        OnPropertyChanged(nameof(PromptModelCapabilityHint));
    ...
    public bool ShowOnlyHardwareUsableLocalLlms
    {
        get => _showOnlyHardwareUsableLocalLlms;
        set
        {
            if (SetProperty(ref _showOnlyHardwareUsableLocalLlms, value))
            {
                ApplyLocalLlmFilters();
            }
        }
