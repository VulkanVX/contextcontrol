...
public sealed partial class ContextControlViewModel
{
    ...
    private const string DiffusersTorchCpuPackage = "torch==2.7.1+cpu";
    ...
    private static string? FirstDependencyFailureLine(DependencyProcessResult result)
    {
        if (result.TimedOut)
        {
            return "timed out; process was stopped";
        ...
        return DependencyFailureLines(result.StandardError).FirstOrDefault()
            ?? DependencyFailureLines(result.StandardOutput).FirstOrDefault();
    }

    private static IEnumerable<string> DependencyFailureLines(string text)
    {
        return (text ?? "")
            ...
            .Select(line => line.Length <= 700 ? line : line[..697] + "...");
    }
        ...
        bool forceManaged = false)
    {
        ...
        if (!forceManaged
            && Directory.Exists(managedDependencyDirectory)
            && !File.Exists(managedPython))
        {
            terminal.Report($"{spec.DisplayName} has a partial managed install without python.exe.");
            terminal.Report($"Removing only ContextControl-managed dependency files: {managedDependencyDirectory}");
            progress.Report(new LocalLlmTransferProgress(
                operation,
                $"Repairing partial managed ContextControl install at {managedDependencyDirectory}.",
                null,
                null,
                null,
                null));
        ...
        pipArgs.AddRange(ResolveManagedPipInstallArguments(spec));
    ...
    private async Task<DependencyInstallResult> InstallDiffusersCpuTorchAsync(
        PythonDependencySpec spec,
        string managedPython,
        string managedDirectory,
        IProgress<string> terminal,
        IProgress<LocalLlmTransferProgress> progress,
        CancellationToken cancellationToken)
    {
        const string operation = "Installing PyTorch CPU runtime";
        var torchArgs = new List<string>
        {
            "-m",
            "pip",
            "install",
            ...
            DiffusersTorchCpuPackage
        };
        terminal.Report($"Installing {DiffusersTorchCpuPackage} from the official CPU wheel index to avoid broken Windows Torch DLL loads.");
        terminal.Report($"> {managedPython} {string.Join(' ', torchArgs)}");
        progress.Report(new LocalLlmTransferProgress(
            operation,
            $"Installing PyTorch CPU wheel into managed ContextControl environment: {managedDirectory}",
            null,
            null,
            null,
            null));

        var torchInstall = await RunProcessForDependencyInstallAsync(
            managedPython,
            torchArgs,
            TimeSpan.FromMinutes(30),
            terminal,
        ...
        {
            var failure = FirstDependencyFailureLine(torchInstall) ?? $"{spec.DisplayName} PyTorch CPU install exited {torchInstall.ExitCode}.";
            return new DependencyInstallResult(false, $"{spec.DisplayName} PyTorch CPU install failed: {failure}");
        }

        return new DependencyInstallResult(true, $"{spec.DisplayName} PyTorch CPU runtime installed.", IsManaged: true);
    }
