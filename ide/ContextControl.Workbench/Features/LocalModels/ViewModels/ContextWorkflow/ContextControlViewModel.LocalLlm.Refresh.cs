...
public sealed partial class ContextControlViewModel
{
    ...
    private static LocalModelRefreshDepth ResolveLocalModelRefreshDepth(object? parameter)
    {
        return parameter is string value
            && value.Equals("fast", StringComparison.OrdinalIgnoreCase)
                ? LocalModelRefreshDepth.Fast
                : LocalModelRefreshDepth.Full;
    }
    ...
    private async Task RefreshLocalModelsAsync(LocalModelRefreshDepth refreshDepth)
    {
        ...
        try
        {
            ...
            if (scanDependencies || ShouldProbeBackendModelCacheAfterRefresh())
            {
                await ApplyBackendModelCacheStatesAsync(cancellationToken, progress, allowPythonProbe: false);
            }
    ...
    private bool ShouldProbeBackendModelCacheAfterRefresh()
    {
        return LocalLlmModels.Any(model => model.UsesDownloadableBackendModel && model.IsBackendDependencyReady);
    }
    ...
    private static IReadOnlyDictionary<string, DependencyRuntimeStatus> DetectCachedBackendDependencyStatuses()
    {
        ...
        foreach (var spec in PythonDependencyEnvironment.AllSpecs)
        {
            ...
            if (spec.Id.Equals("diffusers", StringComparison.OrdinalIgnoreCase)
                && PythonDependencyEnvironment.HasManagedDependency(spec.Id))
            {
                var managedPython = PythonDependencyEnvironment.ManagedPythonExecutable(spec.Id);
                statuses[spec.Id] = new DependencyRuntimeStatus(
                    true,
                    $"{spec.DisplayName} managed venv is present at {managedPython} (cached startup check; runtime validates on full Refresh or generation).",
                    managedPython,
                    IsManaged: true);
                continue;
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var reason = FirstDependencyFailureLine(result) ?? $"exited {result.ExitCode}";
